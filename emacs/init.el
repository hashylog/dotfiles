...
(defcustom my/treemacs-width 32
  "Width of the Treemacs sidebar."
  :type 'integer)

(defcustom my/tab-name-max-width 24
  "Maximum width of a buffer name in the tab line."
  :type 'integer)
...
(defcustom my/echo-area-timeout 2
  "Seconds before informational echo-area messages are cleared."
  :type 'number)
...
(defcustom my/code-prefix-key "C-k"
  "Prefix used for code-related chord shortcuts."
  :type 'string)

(defcustom my/code-keybindings
  '(("C-s" . describe-bindings)
    ("C-c" . comment-region)
    ("C-u" . uncomment-region))
  "Bindings available below `my/code-prefix-key'."
  :type '(alist :key-type string :value-type function))
...
(use-package treemacs
  :init
  (setq treemacs-width my/treemacs-width
        treemacs-position 'left
        treemacs-show-hidden-files t
        treemacs-is-never-other-window nil
        treemacs-persist-file (expand-file-name "treemacs-persist" my/cache-directory)
        treemacs-last-error-persist-file
        (expand-file-name "treemacs-persist-at-last-error" my/cache-directory))
  :config
...
(defun my/clear-echo-area-later (text)
  ...
  (when (and text (not (active-minibuffer-window)))
    (setq my/echo-area-clear-timer
          (run-at-time my/echo-area-timeout nil
                       (lambda ()
  ...
  (with-current-buffer buffer
    (format " %s%s "
            (truncate-string-to-width
             (buffer-name) my/tab-name-max-width nil nil "...")
...
(defun my/apply-keybindings ()
  ...
  (global-set-key (kbd my/code-prefix-key) #'my/code-prefix)
  (dolist (binding my/code-keybindings)
    (define-key my/code-prefix (kbd (car binding)) (cdr binding)))
